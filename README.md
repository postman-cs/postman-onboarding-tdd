...
1. **Checkpoint resume (D9).** After each attempt, a `RepairCheckpointPayload` is written to `.postman-tdd/checkpoint.json` and embedded as `checkpointRef` in the schema-v2 repair sticky-comment marker (and forwarded by schema-v2 failure documents). When `immutable-state-signing-key` is set, the checkpoint is HMAC-signed using the same seam as the immutable-state baseline; a signed checkpoint whose `commit` matches the PR head is **authoritative** (the loop resumes budget accounting from `checkpoint.attempts`). An unsigned checkpoint (no signing key) is **advisory only** — resume is allowed, but `attempts` is recomputed from the repair comment's visible `attemptDetails` timeline, not trusted from the payload. A tampered signature or wrong-head checkpoint causes a restart from `attempts=0`. Schema-v1 failure and repair markers remain readable without checkpoint state.
